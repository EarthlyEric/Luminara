...
Changelogs by EarthlyEric6

---
## v.1.0.pre-release.3.2
- [+/-] `/ping` 資訊 luminara api 顯示buildid
- 
## v.1.0.pre-release.3.1
### 小型更新
- [+/-] Music 細部系統更新
  -[+/-] 暫停播放時，可以進行`/effect` `/skip`
  -[+] 解決`/resume`未註冊到應用命令列表
  -[+] `/skip`現在可以精準跳過到特定曲目(依據`/queue`上的曲目編號)
  -[+] 
## v.1.0.pre-release.3
### 小型更新
- [+] IP地址或網域 查詢 `/addres-lookup`
- [+] LuminaraAPI 引入
## v.1.0.pre-release.2.hotfix.1
skip
## v.1.0.pre-release.2
- [+] 圖片生成 `/image-gen`
- [+] 唬爛文生成 `/bullshit` [Based on Repo](github.com/telunyang/python_bullshit_generator)
- [+] AI對話 `/ask`
- [+/-] `Music`錯誤修復，界面優化
  - `/queue` 翻頁式播放列表顯示
  - 新增音量調節`/volume`
